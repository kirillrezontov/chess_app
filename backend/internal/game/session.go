...
type Snapshot struct {
	...
	MoveList     []string  `json:"move_list"` // SAN or long-algebraic history
...
// goroutine has processed it and returns the resulting snapshot.
...
func (s *Session) handleDrawOffer(c drawOfferCmd) {
	// Simplified: auto-accept for now; a real implementation tracks pending
	// offers per-player and requires both sides. Left as a clear extension
	// point rather than faked complexity.
	s.mu.Lock()
