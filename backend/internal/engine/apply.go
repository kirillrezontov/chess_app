...
// never trust the caller) and mutates the board if so. Returns the applied
// Move (with Captured, IsEnPassant, IsCastle populated) or an error.
func (b *Board) ApplyMove(m Move) (Move, error) {
        ...
        if !found {
                return Move{}, fmt.Errorf("illegal move %s-%s", m.From, m.To)
        }
