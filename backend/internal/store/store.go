...
func (s *Store) GetGameInfo(gameID, userID int64) (*GameInfo, error) {
        ...
        err := s.db.QueryRow(`
                SELECT
                        ...
                        g.increment_sec,
                        g.created_at,
                        coalesce(g.final_fen, '')
                FROM games g
                LEFT JOIN users uw ON uw.id = g.white_id
                LEFT JOIN users ub ON ub.id = g.black_id
                WHERE g.id = $1`, gameID, userID,
        ).Scan(&g.ID, &g.YourColor, &g.OpponentName, &g.Status, &g.Outcome,
                &g.InitialTimeSec, &g.IncrementSec, &g.CreatedAt, &g.FinalFEN)
        if err == sql.ErrNoRows {
                return nil, ErrNotFound
        }
        return g, err
...
func (s *Store) UserGameHistory(userID int64, limit int) ([]HistoryEntry, error) {
        ...
        for rows.Next() {
                var e HistoryEntry
                if err := rows.Scan(&e.ID, &e.YourColor, &e.OpponentName, &e.Status, &e.Outcome,
...
// AddFriend creates a mutual friendship between two users.
// The user_id < friend_id constraint is enforced by the DB CHECK.
func (s *Store) AddFriend(userID, friendID int64) error {
        lo, hi := userID, friendID
        if lo > hi {
                lo, hi = hi, lo
        }
        _, err := s.db.Exec(
...
// RemoveFriend deletes the mutual friendship.
func (s *Store) RemoveFriend(userID, friendID int64) error {
        lo, hi := userID, friendID
        if lo > hi {
                lo, hi = hi, lo
        }
        _, err := s.db.Exec(`DELETE FROM friendships WHERE user_id = $1 AND friend_id = $2`, lo, hi)
        return err
}

// ListFriends returns all friends of a user.
func (s *Store) ListFriends(userID int64) ([]FriendEntry, error) {
        rows, err := s.db.Query(`
        ...
        defer rows.Close()
        out := make([]FriendEntry, 0)
        for rows.Next() {
                var e FriendEntry
                if err := rows.Scan(&e.Username, &e.Rating); err != nil {
                        return nil, err
                }
                out = append(out, e)
        }
...
// SearchUsers finds users by username prefix (excluding self).
func (s *Store) SearchUsers(query string, excludeID int64, limit int) ([]FriendEntry, error) {
        rows, err := s.db.Query(`
...
// GetGameMoves returns all recorded moves for a game, including the FEN
// after each move so the frontend can do step-by-step replay.
func (s *Store) GetGameMoves(gameID int64) ([]MoveRecord, error) {
        rows, err := s.db.Query(
                `SELECT ply, fen_after FROM moves WHERE game_id = $1 ORDER BY move_number, id`, gameID)
        if err != nil {
                return nil, err
        }
        defer rows.Close()
        out := make([]MoveRecord, 0)
        for rows.Next() {
                var m MoveRecord
                if err := rows.Scan(&m.Ply, &m.FenAfter); err != nil {
                        return nil, err
                }
                out = append(out, m)
        }
...
// ---- Direct game creation (for friend invites) ----

// CreateDirectGame creates a game with random colour assignment.
// Returns (gameID, yourColor, error).
func (s *Store) CreateDirectGame(inviterID, friendID int64, initialTimeSec, incrementSec int) (int64, string, error) {
        // Random colour assignment
        whiteID, blackID := inviterID, friendID
        n, err := rand.Int(rand.Reader, big.NewInt(2))
        if err != nil {
                return 0, "", err
        }
        if n.Int64() == 1 {
                whiteID, blackID = friendID, inviterID
        }
        ...
        yourColor := "white"
        if whiteID != inviterID {
                yourColor = "black"
        }
        return id, yourColor, nil
}
